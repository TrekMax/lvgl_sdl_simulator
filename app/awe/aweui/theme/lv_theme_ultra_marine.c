 ...
 *      DEFINES
 *********************/
#define RADIUS_DEFAULT (disp_size == DISP_LARGE ? lv_disp_dpx(theme.disp, 12) : lv_disp_dpx(theme.disp, 0))
...
#define BLACK_COLOR_CARD        lv_color_hex(0x000000)
#define BLACK_COLOR_TEXT        lv_color_hex(0xFFFFFF)

#define TRANSITION_TIME         LV_THEME_ULTRA_MARINE_TRANSITION_TIME
#define BORDER_WIDTH            lv_disp_dpx(theme.disp, 0)
#define OUTLINE_WIDTH           lv_disp_dpx(theme.disp, 0)
...
{
    LV_UNUSED(f);
    if(theme.flags & THEME_MODE_DARK) return lv_color_mix(lv_palette_darken(LV_PALETTE_GREY, 2), color, opa);
...
static void style_init(void)
{
    ...
    if (theme.flags & THEME_MODE_BLACK) {
        color_text = BLACK_COLOR_TEXT;
        color_card = BLACK_COLOR_CARD;
    } else {
        color_scr = theme.flags & THEME_MODE_DARK ? DARK_COLOR_SCR : LIGHT_COLOR_SCR;
        color_text = theme.flags & THEME_MODE_DARK ? DARK_COLOR_TEXT : LIGHT_COLOR_TEXT;
        color_card = theme.flags & THEME_MODE_DARK ? DARK_COLOR_CARD : LIGHT_COLOR_CARD;
        color_grey = theme.flags & THEME_MODE_DARK ? DARK_COLOR_GREY : LIGHT_COLOR_GREY;
    }
    ...
    lv_color_t sb_color = (theme.flags & THEME_MODE_DARK) ? lv_palette_darken(LV_PALETTE_GREY,
    ...
    lv_style_set_pad_all(&styles->scrollbar, lv_disp_dpx(theme.disp, 0));
    lv_style_set_width(&styles->scrollbar,  lv_disp_dpx(theme.disp, 0));
    ...
    if(!(theme.flags & THEME_MODE_DARK)) {
    ...
    lv_style_set_text_color(&styles->ta_placeholder, (theme.flags & THEME_MODE_DARK) ? lv_palette_darken(LV_PALETTE_GREY,
...
lv_theme_t * lv_theme_ultra_marine_init(lv_disp_t * disp, lv_color_t color_primary, lv_color_t color_secondary, uint32_t theme_mode, const lv_font_t * font)
{

    /*This trick is required only to avoid the garbage collection of
     *styles' data if LVGL is used in a binding (e.g. Micropython)
     *In a general case styles could be in simple `static lv_style_t my_style...` variables*/
    // if(!lv_theme_ultra_marine_is_inited()) {
    if(!inited) {
        inited = false;
    ...
    case THEME_MODE_BLACK:
        theme.flags = theme_mode;
        break;
    default:
        theme.flags = THEME_MODE_LIGHT;
        break;
    }
