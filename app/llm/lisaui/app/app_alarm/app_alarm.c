...
#if CONFIG_LISAUI_APP_ALARM_MACRO_ENABLE

// 声明[模板]应用需要实现的接口
LISAUI_DECLARE_APP_FUNC(alarm, create, destroy, enter, exit, get_page);
// 定义[模板]应用
LISAUI_DEFINE_APP(alarm, UI_APP_ID_ALARM, "闹钟", "Alarm", &icon_img_app_store_png, {});

// App 模板宏原型在 common/lisaui_app_manager.h 中定义
// 定义[模板]应用功能实现, 用于创建、销毁、进入、退出、获取页面
#define LISAUI_APP_ENTITY_ALARM                                                                                     \
    {                                                                                                                  \
        if (g_app_alarm != NULL) {                                                                                  \
        ...
        _lisaui_set_style_container(g_app_alarm, lv_color_hex(0x000000), 255, lv_color_hex(0x000000), 0, 0);        \
                                                                                                                       \
        lv_obj_t *label = lv_label_create(g_app_alarm);                                                             \
...
lisaui_err_t LISAUI_DEFINE_APP_FUNC(alarm, create, void *parent, {LISAUI_APP_ENTITY_ALARM});
lisaui_err_t LISAUI_DEFINE_APP_FUNC(alarm, destroy, void, {
    LVGL_OBJ_SAFE_DEL(g_app_alarm);
    return LISAUI_ERR_OK;
});
lisaui_err_t LISAUI_DEFINE_APP_FUNC(alarm, enter, void, { return LISAUI_ERR_OK; });
lisaui_err_t LISAUI_DEFINE_APP_FUNC(alarm, exit, void, { return LISAUI_ERR_OK; });
void *LISAUI_DEFINE_APP_FUNC(alarm, get_page, void, { return g_app_alarm; });
