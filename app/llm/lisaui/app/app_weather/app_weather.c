...
#if CONFIG_LISAUI_APP_WEATHER_MACRO_ENABLE

// 声明[天气]应用需要实现的接口
LISAUI_DECLARE_APP_FUNC(weather, create, destroy, enter, exit, get_page);
// 定义[天气]应用
LISAUI_DEFINE_APP(weather, UI_APP_ID_WEATHER, "天气", "Weather", &icon_img_app_store_png, {});

// App 天气宏原型在 common/lisaui_app_manager.h 中定义
// 定义[天气]应用功能实现, 用于创建、销毁、进入、退出、获取页面
#define LISAUI_APP_ENTITY_WEATHER                                                                                     \
    {                                                                                                                  \
        ...
        _lisaui_set_style_container(g_app_weather, lv_color_hex(0x000000), 255, lv_color_hex(0x000000), 0, 0);        \
                                                                                                                       \
        lv_obj_t *label = lv_label_create(g_app_weather);                                                             \
...
lisaui_err_t LISAUI_DEFINE_APP_FUNC(weather, create, void *parent, {LISAUI_APP_ENTITY_WEATHER});
lisaui_err_t LISAUI_DEFINE_APP_FUNC(weather, destroy, void, {
    LVGL_OBJ_SAFE_DEL(g_app_weather);
    return LISAUI_ERR_OK;
});
lisaui_err_t LISAUI_DEFINE_APP_FUNC(weather, enter, void, { return LISAUI_ERR_OK; });
lisaui_err_t LISAUI_DEFINE_APP_FUNC(weather, exit, void, { return LISAUI_ERR_OK; });
void *LISAUI_DEFINE_APP_FUNC(weather, get_page, void, { return g_app_weather; });
